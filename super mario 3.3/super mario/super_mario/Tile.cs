...
namespace super_mario
{
    public class Tile
    {
        ...
        public enum Specials { Destroyable, Mushroom, Normal};
        ...
        public void Update(GameTime gameTime, ref Player player, ref Layer layer)
        {
            ...
            {
                if (player.Rect.Intersects(rect) && state == State.Solid && specials == Specials.Mushroom && player.PrevPosition.Y > position.Y && player.Velocity.Y < 0)
                {
                    activated = true;
                ...
                if (player.Rect.Intersects(rect) && this.state == State.Solid && this.specials == Specials.Destroyable && player.Position.Y > position.Y && player.Velocity.Y < 0)
                {
                    destroyed = true;
                    player.Velocity = new Vector2(player.Velocity.X, 0);
                }

                //if (player.Position.X < this.position.X + 16 && player.Position.X > this.position.X)
                //{
                //}
            }
